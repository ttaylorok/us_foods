# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 15:19:49 2020
...
df1 = pd.read_csv("all_restaurants_usa_overpass_v1.csv")
df2 = pd.read_csv("all_restaurants_usa_overpass_v2.csv")
df3 = pd.read_csv("all_restaurants_usa_overpass_v3.csv")
df4 = pd.read_csv("all_restaurants_usa_overpass_v4.csv")
df5 = pd.read_csv("all_restaurants_usa_overpass_v5.csv")
df6 = pd.read_csv("all_restaurants_usa_overpass_v6.csv")
df7 = pd.read_csv("all_restaurants_usa_overpass_v7.csv")
df8 = pd.read_csv("all_restaurants_usa_overpass_v8.csv")
df9 = pd.read_csv("all_restaurants_usa_overpass_v9.csv")

dfc = df1.append(df2).append(df3).append(df4).append(df5).append(df6).append(df7).append(df8).append(df9)

dfc.drop_duplicates(inplace=True)

dfc.columns = ["id","name","cuisine","lat","lon","state"]


filt = dfc["cuisine"] != "none"
dfc = dfc.loc[filt,:]

dfc["cuisine_new"] = dfc["cuisine"]
dfc['row_id'] = 0
dfc["cuisine_group"] = "none"
row_id = 0
"""
...
for index, row in dfc.iterrows():
    ...
    if "mexican" in c or "taco" in c or "burrito" in c or "taqueria" in c or "yucatan" in c or "taquiera" in c:
        ...
        new = "other"
    dfc.iloc[row_id,6] = new
    dfc.iloc[row_id,7] = row_id
    dfc.iloc[row_id,8] = group
    row_id += 1


#dfc.to_csv("all_restaurants_compiled_full.csv")
dfc.to_csv("all_restaurants_compiled_full.csv")

# filt2 = dfc["cuisine_new"] == "other"
# dfc.loc[filt2,"cuisine"]

# g = dfc.groupby(["state","cuisine_new"])["cuisine_new"].count()
# g2 = g.groupby("state")["cuisine_new"].count()

# bins = dfc.groupby("cuisine_new")["cuisine"].unique()
